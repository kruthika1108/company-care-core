...
export function downloadPayslipPDF(opts: {
  ...
  autoTable(doc, {
    ...
    body: [
      ["Basic", fmt(Number(slip.basic))],
      ["HRA", fmt(Number(slip.hra))],
      ["Allowances", fmt(Number(slip.allowances))],
      [{ content: "Gross", styles: { fontStyle: "bold" } }, { content: fmt(Number(slip.gross)), styles: { fontStyle: "bold" } }],
    ],
    theme: "grid",
  ...
  autoTable(doc, {
    ...
    body: [
      ["Provident Fund (12%)", fmt(Number(slip.pf))],
      ["Income Tax", fmt(Number(slip.tax))],
      ["Other", fmt(Number(slip.other_deductions))],
      [
        { content: "Total Deductions", styles: { fontStyle: "bold" } },
        { content: fmt(Number(slip.pf) + Number(slip.tax) + Number(slip.other_deductions)), styles: { fontStyle: "bold" } },
      ],
    ],
    theme: "grid",
    headStyles: { fillColor: [20, 30, 48], textColor: 255 },
